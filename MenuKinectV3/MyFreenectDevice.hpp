	...
	if (isTouchingSurface(it->x, it->y))
	  {
	    ...
	    f.y = it->y - surfacePoints[BOTTOM_RIGHT].y;
	    // f.x = x;
	    // f.y = y;
	    // f.x -= surfacePoints[BOTTOM_RIGHT].x;
	    // f.y -= surfacePoints[BOTTOM_RIGHT].y;
	    f.x *= -1;
	    f.y *= -1;
	    f.x *= 800;
	    f.y *= 600;
    ...
    if (m_new_rgb_frame && !calibrated && !surface)
      {
        ...
        while (Threshold < 200)
          {
            cvThreshold(tmpGray, gray, 130, 255, CV_THRESH_BINARY);
