...
for k = 1:ccount
  ...
  [calib0.rK{k},calib0.rkc{k},Rext{k},text{k},calib0.color_error_var(k),~] = rgb_calib(rKc,kc0,rRc,rtc,rgb_grid_p{k},rgb_grid_x{k});
  ...
  if(k==1)
    ...
    rR = zeros(3,icount);
    rt = zeros(3,icount);
    valid_indices = find(~(cellfun(@(x) isempty(x),Rext{k}) | cellfun(@(x) isempty(x),calib0.Rext)));
    for i=valid_indices
      Ri = calib0.Rext{i}*Rext{k}{i}';
      rR(:,i) = rotationpars( Ri );
      rt(:,i) = calib0.text{i} - Ri*text{k}{i};
    end
    calib0.rR{k} = rotationmat( mean(rR(:,valid_indices),2) );
    calib0.rt{k} = mean(rt(:,valid_indices),2);
...
%     error_abs = [error_abs, errori];
    ...
    p_rel = project_points_k(rgb_grid_x{k}{i},calib0.rK{k},calib0.rkc{k},R,t);
