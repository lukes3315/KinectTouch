...
function do_calib(use_depth_distortion)
...
if(isempty(max_depth_sample_count))
  max_depth_sample_count = 60000;
end
fprintf('Using a maximum of %d disparity samples. Decrease ''max_depth_sample_count'' if you run out of memory.\n',max_depth_sample_count);
...
while(~converged)
  ...
  options.use_fixed_dc_alpha = ~any(calib_old.dc_beta(:));
  ...
  if(use_depth_distortion)
    ...
    [calib_new.dc_beta]=calib_distortion_beta(calib_new,calib_new.Rext, calib_new.text, dataset_path, dfiles, depth_plane_mask);
%       [calib_new.dc_woffset]=calib_distortion_woffset(calib_new,calib_new.Rext, calib_new.text, dataset_path, dfiles, depth_plane_mask);
  end
...
for k=1:ccount
  plot_rgb_errors(final_calib,k);
end
plot_depth_errors(final_calib);
