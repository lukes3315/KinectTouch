...
kinect_capture_app::kinect_capture_app():
    ...
    depth_texture(512,1024),
    depth_texture_sub(depth_texture,cv::Range(0,480),cv::Range(0,640)),
    save_pending(false)
{
}
...
void kinect_capture_app::run(int argc, char **argv) {
    ...
    video_texture = cv::Mat(1024,1024,freenect.video_frame->type());
    if(video_frame->cols > video_texture.cols)
        video_texture_sub = cv::Mat(video_texture,cv::Range(0,video_frame->rows*video_texture.cols/video_frame->cols),cv::Range(0,video_texture.cols));
    else
        video_texture_sub = cv::Mat(video_texture,cv::Range(0,video_frame->rows),cv::Range(0,video_frame->cols));
...
void kinect_capture_app::do_glutIdle()
{
	...
	if(is_depth_new)
	{
    	...
    	show_mat.convertTo(depth_texture_sub,cv::DataDepth<uchar>::value, 255.0/(maxval-minval), -255*minval/(maxval-minval));
	}

	if(is_video_new) {
        if(video_texture_sub.cols != video_frame->cols)
    		cv::resize(*video_frame, video_texture_sub, cv::Size(video_texture_sub.cols, video_texture_sub.rows));
        else
            video_frame->copyTo(video_texture_sub);
	}
...
void kinect_capture_app::do_glutDisplay() {
	...
	//CHANGE: updated texture size to avoid OpenGL power of two problem (coordinates below also updated)
	glTexImage2D(GL_TEXTURE_2D, 0, 1, depth_texture.cols, depth_texture.rows, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, depth_texture.data);

    float r=depth_texture_sub.cols / (float)depth_texture.cols;
    float b=depth_texture_sub.rows / (float)depth_texture.rows;

	glBegin(GL_TRIANGLE_FAN);
	glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
	glTexCoord2f(0, 0); glVertex3f(0,0,0);
	glTexCoord2f(r, 0); glVertex3f(640,0,0);
	glTexCoord2f(r, b); glVertex3f(640,480,0);
	glTexCoord2f(0, b); glVertex3f(0,480,0);
	glEnd();

	glBindTexture(GL_TEXTURE_2D, gl_rgb_tex);
	GLint bytes;
	GLenum format;
	if(video_texture.channels() == 1) {
		bytes = 1;
		format = GL_LUMINANCE;
	} else {
		bytes = 3;
	...
	glTexImage2D(GL_TEXTURE_2D, 0, bytes, video_texture.cols, video_texture.rows, 0, format, GL_UNSIGNED_BYTE, video_texture.data);

    r = video_texture_sub.cols / (float)video_texture.cols;
    b = video_texture_sub.rows / (float)video_texture.rows;

	glBegin(GL_TRIANGLE_FAN);
	glColor4f(1.0f, 1.0f, 1.0f, 1.0f);
	glTexCoord2f(0, 0); glVertex3f(640,0,0);
	glTexCoord2f(r, 0); glVertex3f(1280,0,0);
	glTexCoord2f(r, b); glVertex3f(1280,480,0);
	glTexCoord2f(0, b); glVertex3f(640,480,0);
