#ifndef MYFREENECTDEVICE_HPP
#define MYFREENECTDEVICE_HPP
	...
	if (calib == true)
	  {
	    ...
	    surfaceYCoef = tops[0][1] - tops[1][1];
	    surfaceYCoef = (float)600.f / surfaceYCoef;
	    surfaceXCoef = tops[3][0] - tops[1][0];
	    surfaceXCoef = (float)800.f / surfaceXCoef;
  ...
  void showTouchSurface()
  {
	...
	if ((*it)->x == (*it2)->x &&
	    (*it)->y == (*it2)->y && (*it)->z < 500)
	  {
	    if ((*it2)->z < (*it)->z - 10 && (*it2)->z > (*it)->z - 20)
	      {
		if ((*it2)->z < matAllDist[tops[0][1]][tops[0][0]]
      ...
      while (j < 640)
	{
	  if (matrixBin[i][j] != 0)
		  ...
		  && matrixBin[i - 1][j + 1] != 0)
		{
		  ...
		  if (tmpY < 0)
		    tmpY *= -1;
		  fingerList.push_back(new coords(tmpX, tmpY, matrixBin[i][j]));
		  glColor3f(1, 0, 0);
